...
Private Sub cmdEndTest_Click()
  ...
  Ans = MsgBox("Do you want to End this Test.", vbYesNo, "Confirmation")
  If Ans = 6 Then
    frmOnlineTesting.Visible = True
    Call Calculate
    Unload frmOnlineTesting
    conn.Execute "Delete * from QuestionPaper"
    conn.Execute "Delete * from QuestionStatus"
    Unload frmReview
  End If
End Sub
...
Private Sub Form_Load()
   ...
   QStatus.Open "QuestionStatus", conn, adOpenDynamic, adLockOptimistic
   QPaper.Open "QuestionPaper", conn, adOpenDynamic, adLockOptimistic
   ...
   txtMin.Text = frmOnlineTesting.txtMin.Text
   txtSec.Text = frmOnlineTesting.txtSec.Text
End Sub
...
Private Sub Command1_Click(Index As Integer)
   frmOnlineTesting.Visible = True
   For i = 1 To 4
     frmOnlineTesting.txtAnswer(i).ForeColor = &H0&
     frmOnlineTesting.optAnswer(i).Value = False
   Next
   
   frmOnlineTesting.chkBookmark.Value = 0
   frmOnlineTesting.txtQuestionNo = Command1(Index).Caption
   QPaper.MoveFirst
   QStatus.MoveFirst
   ...
   If QStatus!vStatus = "B" Then
      frmOnlineTesting.chkBookmark.Value = 1
      ind = 0
    '  QStatus.MoveFirst
    '  QStatus.Move (Command1(Index).Caption - 1)
      ...
      ind = QStatus!vAnswer
      If Not ind = "" Then
        frmOnlineTesting.optAnswer(QStatus!vAnswer).Value = 1
        frmOnlineTesting.txtAnswer(QStatus!vAnswer).ForeColor = &HFF&
      End If
   ElseIf QStatus!vStatus = "S" Then
     ...
     ind = QStatus!vAnswer
     frmOnlineTesting.optAnswer(ind) = True
     frmOnlineTesting.txtAnswer(ind).ForeColor = &HFF&
   End If
    '''''''''''''''''''''''''''''''''''''''''''''''''
   Unload Me
End Sub
...
End Sub
Function Fill()
   frmOnlineTesting.txtQuestion.Text = QPaper!vQuestion
   frmOnlineTesting.txtAnswer(1) = QPaper!vOption1
   frmOnlineTesting.txtAnswer(2) = QPaper!vOption2
   frmOnlineTesting.txtAnswer(3) = QPaper!vOption3
   frmOnlineTesting.txtAnswer(4) = QPaper!vOption4
   frmOnlineTesting.txtPositive.Text = QPaper!iPositive
   frmOnlineTesting.txtNegative.Text = QPaper!iNegative
   frmOnlineTesting.txtQuestionNo.Text = QPaper!iquestionno
End Function
